...
@Component({
  selector: 'app-fabrik-constraint',
  standalone: true,
  imports: [CodeBlockComponent, CommonModule],
  templateUrl: './fabrik-constraint.component.html',
  styleUrl: './fabrik-constraint.component.scss'
})


export class FabrikConstraintComponent implements AfterViewInit {
  ...
  chainLength = 5; // Number of joints 
  ...
  ngAfterViewInit(): void {
    ...
    if (isPlatformBrowser(this.platformId)){
      this.loadAlgorithmContent("fabrik-constraint");
      this.startAnimation();
  ...
  drawSimpleConstraint1(): void {
    ...
    this.joints[this.joints.length - 1].x = center.x;
    this.joints[this.joints.length - 1].y = center.y;
    for(let i =this.joints.length-1; i>0; i--) {
      const joint = this.joints[i];
      const prevJoint = this.joints[i-1];

      const constrainedPos = this.constrainDistance(prevJoint, joint, this.jointLength);
      this.joints[i-1].x = constrainedPos.x;
      this.joints[i-1].y = constrainedPos.y;
    }
